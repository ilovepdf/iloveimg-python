#!/usr/bin/env python3
"""
Verifies that the iloveimg package is installed in the expected location.
...
Arguments:
    global  - Checks that iloveimg is installed in site-packages or dist-packages.
    user    - Checks that iloveimg is installed in the user site (.local).
...
def check_import(expected_path):
    try:
        ...
        iloveimg_path = iloveimg.__file__
        print(f"Install location: {iloveimg_path}")
        if expected_path not in iloveimg_path:
            print(f"ERROR: Not installed in {expected_path}")
            sys.exit(1)
    except ImportError:
        print("ERROR: iloveimg package not found.")
        sys.exit(1)
