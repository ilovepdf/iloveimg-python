"""Base class for iLoveIMG Task integration tests.
...
class BaseTaskIntegrationTest(Generic[T]):
    ...
    iLoveIMG Task classes (e.g., CompressTask, ProtectTask, etc.).

    Attributes:
        public_key (str): iLoveIMG public API key from environment.
    ...
    public_key: str = os.environ.get("ILOVEIMG_PUBLIC_KEY", "")
    secret_key: str = os.environ.get("ILOVEIMG_SECRET_KEY", "")
    ...
    def setup_class(self):
        ...
        if not self.public_key or not self.secret_key:
            pytest.skip("iLoveIMG API credentials not found in environment variables.")
